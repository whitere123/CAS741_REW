...
\begin{document}
	...
	\begin{description}
		...
		a set of 
		points on the complex plane. \wss{Why is this an unlikely change?  Why 
		does the
			output have to go to the screen?  It is an easy (and possibly 
			useful change)
			to send the output to a file, or to make the output a table of 
			values, rather
			than a plot.}
	...
	\begin{description}
		...
		verifies 
		that 
		the input variables satistfy \wss{spell check} the constraints in the 
	...
	\begin{description}
		...
		theoretical eigenvalues. \wss{Do you need a specific module for this?
			...
			not relative error?  Isn't this a one line formula in the code?  
			Maybe
	...
	\begin{description}
		...
		eigenfunction derivatives. \wss{How are these different from input?
			...
			constant between runs?  Are they like configuration parameters for 
			the
			numerical algorithm?}
	...
	\begin{figure}[h!]
		...
		Eigen 
			Value Equations (M5)};
		\node [block, left of= SpecErr, node distance= 7cm] (control) {Control 
		(M14)};
		\node [block, left of=specmat,node distance=3cm] (diag) {Diagonal 
		Matrix 
			(M9)} ;
		% Draw edges
	...
	\wss{Why doesn't the Control module use any other modules?  Try to draw your
		uses relation so that all arrows are down.  Why does numerical 
		parameters use
		the Diagonal Matrix (and other services)?  I had the impression that 
		numerical
		parameters were configuration constants.  Why does numerical parameters 
		use
		input parameters?  I'm sorry, but I really don't understand your 
		design.}
	\newpage
	
	\wss{Your bibliography didn't compile properly.  I had to modify the path.  
	I
		had to do the same thing for the Comments.tex file.}
	
	\wss{Throughout this document you hard code in your references to other 
	modules
		(M2, M9 etc.).  You should make your life easier and use labels and 
		references
		in \LaTeX{}.}
